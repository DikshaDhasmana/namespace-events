...
const ContactSection = () => {
  ...
  return (
    <section id="contact" className="scroll-section bg-background text-foreground relative overflow-hidden section-transition">
        ...
        <div className="w-full max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8 max-w-4xl mx-auto">
            {ctaSections.map((section, index) => (
              <Dialog key={index} open={openDialog === section.action.label} onOpenChange={(open) => setOpenDialog(open ? section.action.label : null)}>
                <DialogTrigger asChild>
                  <div className="bg-card border border-border rounded-2xl p-4 progressive-reveal hover:border-primary/40 transition-all duration-300 group cursor-pointer hover:bg-primary/5 aspect-square flex flex-col items-center justify-center text-center scale-75 md:scale-50 origin-center">
                    <div className="w-8 h-8 bg-primary/20 border border-primary/30 rounded-lg flex items-center justify-center text-primary group-hover:bg-primary/30 transition-colors mb-2">
                      {section.icon}
                    </div>
                    <h3 className="text-sm font-sora font-bold text-foreground leading-tight">
                      {section.title}
                    </h3>
                  </div>
                </DialogTrigger>
                ...
                <DialogContent className="bg-card border-border text-foreground max-w-md">
                  <div className="p-2">
                    ...
                    <div className="flex items-start space-x-4 mb-4">
                      <div className="flex-shrink-0 w-14 h-14 bg-primary/20 border border-primary/30 rounded-xl flex items-center justify-center text-primary">
                        {section.icon}
                      </div>
                      <div>
                        <h3 className="text-xl font-sora font-bold text-foreground mb-2">
                          {section.title}
                        </h3>
                    ...
                    <div className="space-y-3">
                          ...
                          <div className="grid grid-cols-1 gap-3">
                            {section.action.platforms?.map((platform, idx) => (
                              <Button 
                                key={idx}
                                variant="outline"
                                className="flex items-center justify-between bg-namespace-purple/5 border-namespace-purple/20 text-namespace-black hover:bg-namespace-purple/20 hover:border-namespace-purple p-4 h-auto"
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            {section.action.platforms?.map((platform, idx) => (
                              <Button 
                                key={idx}
                                variant="outline"
                                className="flex items-center justify-between bg-namespace-purple/5 border-namespace-purple/20 text-namespace-black hover:bg-namespace-purple/20 hover:border-namespace-purple p-3 h-auto"
                                onClick={() => window.open(platform.url, '_blank')}
                              >
                                <div className="flex items-center space-x-3">
                                  {platform.icon}
                      ...
                      {section.action.type === 'newsletter' && (
                        <div className="space-y-3">
                          <div className="bg-namespace-purple/5 border border-namespace-purple/10 rounded-lg p-3 mb-3">
                          ...
                          <Button 
                            className="w-full bg-namespace-purple hover:bg-namespace-purple/90 text-namespace-white font-semibold"
        ...
        <div className="border-t border-border pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-6">
            ...
            <div className="flex items-center">
              <img 
                src={theme === 'dark' ? "/logos/horizontal-white-dark.png" : "/logos/horizontal-primary-light.png"}
                alt="NAMESPACE"
                className="h-5"
              />
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <Link to="/terms-of-use" className="text-muted-foreground hover:text-primary text-sm transition-colors">
                Terms of Use
              </Link>
              <Link to="/branding" className="text-muted-foreground hover:text-primary text-sm transition-colors">
                Brand Guidelines
              </Link>
            </div>
          </div>
