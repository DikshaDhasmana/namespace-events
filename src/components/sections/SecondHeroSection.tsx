...
const SecondHeroSection = () => {
  ...
  return (
    <section id="solution" ref={ref} className="scroll-section bg-background text-foreground relative overflow-hidden">
        ...
        <div className="w-full max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
            {features.map((feature, index) => (
              <div 
                ...
                className="group relative h-20 sm:h-24 lg:h-28 rounded-2xl border border-border hover:border-primary/40 hover:shadow-sm transition-all duration-300 overflow-hidden cursor-pointer bg-card"
              ...
              >
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center p-3 sm:p-4 transition-all duration-300 lg:group-hover:opacity-0">
                  <div className="w-3 h-3 sm:w-4 sm:h-4 bg-primary rounded-full mb-2 sm:mb-3"></div>
                  <h3 className="text-xs sm:text-sm lg:text-base font-sora font-semibold text-center text-foreground leading-tight">
                    {feature.title}
                  </h3>
                </div>

                {/* Hover state - Description (Desktop only) */}
                <div className="hidden lg:flex absolute inset-0 bg-primary rounded-2xl p-3 sm:p-4 flex-col justify-center transform scale-95 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all duration-300">
                  <h3 className="text-xs sm:text-sm font-sora font-bold text-center text-primary-foreground mb-1 sm:mb-2">
                    {feature.title}
                  </h3>
                  <p className="text-primary-foreground/90 text-xs text-center leading-relaxed">
                    {feature.description}
                  </p>
                </div>
              </div>
          ...
          <Dialog open={!!selectedFeature} onOpenChange={() => setSelectedFeature(null)}>
            <DialogContent className="sm:max-w-md bg-card">
              <DialogHeader>
                <DialogTitle className="text-primary font-sora text-xl">
